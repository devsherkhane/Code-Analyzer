                  ...
                  <tbody>
                    <template v-for="(issue, idx) in group" :key="fName + idx">
                      <tr class="row-clickable" @click="toggleIssueExpand(fName + idx)">
                        ...
                        <td class="cell-mono" style="font-size:0.78rem">{{ issue.defect_type }}</td>
                        ...
                        <td colspan="4" style="padding:0">
                          <div class="expandable-content">
                            <div class="expand-section">
                              ...
                              <div class="expand-body" v-html="formatMarkdown(issue.rationale)"></div>
                            </div>
                            <div v-if="issue.is_real_issue && issue.suggestion" class="expand-section expand-fix">
                              <div class="expand-label fix-label">Recommended Fix</div>
                              <div class="expand-body" v-html="formatMarkdown(issue.suggestion)"></div>
                              <div v-if="issue.fixed_code_snippet" class="code-block" style="margin-top:0.75rem">
                                <div class="code-block-header">Suggested Fix</div>
                                <pre><code>{{ issue.fixed_code_snippet }}</code></pre>
                              </div>
                            </div>
                    ...
                    <div v-if="issuesPanelOpen" class="issues-panel-body">
                      ...
                      >
                        <div class="ws-issue-top">
                          ...
                          <span class="ws-issue-type">{{ issue.defect_type }}</span>
                        ...
                        <p class="ws-issue-rationale">{{ issue.rationale }}</p>
                        <div v-if="activeFileIssue === idx" class="ws-issue-expanded">
                          <div v-if="issue.suggestion" class="ws-issue-fix">
                            <div class="ws-fix-label">Recommended Fix</div>
                            <p>{{ issue.suggestion }}</p>
                            <div v-if="issue.fixed_code_snippet" class="code-block" style="margin-top:0.5rem">
                              <div class="code-block-header">Suggested Fix</div>
                              <pre><code>{{ issue.fixed_code_snippet }}</code></pre>
                            </div>
                          </div>
...
export default {
  name: "AIReport",
  components: { SourceViewer, FileTree },
  ...
  computed: {
    ...
    selectedFileIssues() {
      ...
      const issues = [];
      (this.selectedFile.ui_accessibility_analysis || []).forEach(i => {
        if (i.is_real_issue) issues.push({ ...i, _source: 'UI/A11y' });
      });
      (this.selectedFile.ai_analysis || []).forEach(i => {
    ...
    async fetchReport() {
      ...
      try {
        const [reportRes, archRes] = await Promise.all([
          axios.get(`http://localhost:8081/ai_report?_t=${Date.now()}`),
          axios.get(`http://localhost:8081/ai_architecture?_t=${Date.now()}`).catch(() => ({ data: null }))
        ]);
    ...
    focusIssue(issue, idx) {
      ...
      if (startLine) {
        ...
        if (issue.fixed_code_snippet) {
          count = Math.max(1, issue.fixed_code_snippet.trim().split('\n').length);
        }
