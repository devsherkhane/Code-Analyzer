            ...
            switch (message.command) {
                ...
                case 'applyFix': {
                    try {
                        ...
                        await vscode.commands.executeCommand('vscode.diff', originalUri, previewUri, '⚡ AI Fix Preview  (Original ← → Fixed)', { preview: true });
                        const confirm = await vscode.window.showInformationMessage('Review the diff above. Apply this AI fix?', { modal: false }, 'Apply Fix', 'Cancel');
                        // Clean up temp file
                        try {
                        ...
                        if (errors.length > 0) {
                            ...
                            const action = await vscode.window.showWarningMessage(`⚠️ Fix applied but ${errors.length} compilation error(s) detected: ${errorMessages}`, 'Undo Fix (Rollback)', 'Keep Anyway');
